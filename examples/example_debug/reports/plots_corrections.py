...
data, header = read_csv_file('all_data/reports_from_2020_09_28/report_of_2020_09_28_15_18_49.dat', 1)
cut_data = 1
...
ax1.set_title('Periodicity Busy waiting (max:  ' + str(max(data[:,header[0].index('kin.periodicity')])) +  ' ms, min: '+ str(min(data[:,header[0].index('kin.periodicity')]))+  ' ms, mean: '+ str(np.mean(data[:,header[0].index('kin.periodicity')]))  + ' ms)')

# ax2.plot(data[:,header[0].index('TimeStamp')],data[:,header[0].index('traj_gen.desired_positions.6')]*180/3.1416)
# ax2.plot(data[:,header[0].index('TimeStamp')],data[:,header[0].index('kin.sent_setpoints.6')]*180/3.1416)
# ax2.set_ylabel('Periodicity [ms]')
# ax2.set_title('Setpoints [deg]')
# ax2.set_xlabel('Time [s]')
# ax2.legend(['desired setpoints, smoothed setpoints'])
